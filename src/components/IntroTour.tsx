...
const STEPS: Step[] = [
  ...
  { target: null, label: "Feature ideas", title: "Where this could go next.", body: "A few directions I would be excited to build next:", ideas: ["Opt in to news from the companies you care about", "Get a heads-up when a fitting opening appears", "Celebrate alumni moves, new roles and funding wins", "Save a personal shortlist of people, places and events", "Make this a proper iOS app", "Turn scattered group chats into one exchange forum, sorted by topic", "Add a practical AI video-course section — I’d be happy to help make it"] },
      ...
      <section ref={bubble} className={`tutorial-bubble relative w-full border border-yellow/30 bg-ink-2 p-5 shadow-2xl sm:p-6 ${intro ? "max-w-xl rounded-2xl" : "max-w-sm rounded-xl"}`}>
        {intro ? (
          ...
          <div className="animate-rise">
            ...
            {current.ideas && <div className="mt-4 grid grid-cols-2 gap-2">{current.ideas.map((idea, index) => <div key={idea} className={`min-h-16 rounded-lg border border-line-soft bg-ink/50 p-2.5 ${index === current.ideas!.length - 1 ? "col-span-2" : ""}`}><span className="block text-[9px] font-semibold tracking-[0.16em] text-yellow">0{index + 1}</span><p className="mt-1 text-[11px] leading-snug text-cream/85">{idea}</p></div>)}</div>}
