      ...
      <section ref={bubble} className={`tutorial-bubble relative w-full border border-yellow/30 bg-ink-2 p-5 shadow-2xl sm:p-6 ${intro ? "max-w-xl rounded-2xl" : "max-w-sm rounded-xl"}`}>
        {intro ? (
          ...
          <div className="animate-rise">
            ...
            {current.ideas && <div className="mt-4 grid grid-cols-2 gap-2">{current.ideas.map((idea, index) => <div key={idea} className="min-h-16 rounded-lg border border-line-soft bg-ink/50 p-2.5"><span className="block text-[9px] font-semibold tracking-[0.16em] text-yellow">0{index + 1}</span><p className="mt-1 text-[11px] leading-snug text-cream/85">{idea}</p></div>)}</div>}
