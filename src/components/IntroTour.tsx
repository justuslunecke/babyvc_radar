...
type Step = { target: string | null; label: string; title: string; body: string };
...
export default function IntroTour() {
  ...
  useEffect(() => {
    if (!open || step < 0 || !STEPS[step]?.target) return;
    const align = () => {
      const target = document.querySelector(`[data-tour-target="${STEPS[step].target}"]`);
